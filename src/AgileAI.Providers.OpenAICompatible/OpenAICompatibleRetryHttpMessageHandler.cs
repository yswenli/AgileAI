...
public class OpenAICompatibleRetryHttpMessageHandler : DelegatingHandler
{
    private readonly OpenAICompatibleOptions _options;
    private readonly ILogger<OpenAICompatibleRetryHttpMessageHandler>? _logger;

    public OpenAICompatibleRetryHttpMessageHandler(
        OpenAICompatibleOptions options,
        ILogger<OpenAICompatibleRetryHttpMessageHandler>? logger = null)
    {
        _options = options;
        _logger = logger;
    }
        ...
        while (true)
        {
            ...
            catch (HttpRequestException ex)
            {
                if (retryAttempt < _options.MaxRetryCount)
                {
                    ...
                    _logger?.LogWarning(
                        ex,
                        "Network error occurred. Retrying attempt {RetryAttempt}/{MaxRetryCount} after {Delay}ms",
                        retryAttempt, _options.MaxRetryCount, delay.TotalMilliseconds);
