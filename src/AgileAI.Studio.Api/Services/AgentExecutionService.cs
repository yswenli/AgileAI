    ...
    StudioStreamingTurnFinalizer streamingTurnFinalizer)
{
    ...
    public async Task StreamMessageAsync(Guid conversationId, string content, HttpResponse response, CancellationToken cancellationToken)
    {
        ...
        await StudioSseWriter.WriteAsync(response, "message-created", new
        {
        ...
        try
        {
            if (agent.EnableSkills)
            {
                ...
                {
                    var preparedHistory = SkillPromptHelper.PrepareHistoryForSkill(BuildRequestHistory(conversation, agent).ToList(), resolvedSkill.Manifest!).ToList();
                    ...
                    var skillSession = await CreateSessionAsync(conversation, agent, runtime.RuntimeModelId, chatClient, preparedHistory, cancellationToken);
        ...
        }, cancellationToken))
        {
            switch (update)
            {
                ...
                case UsageUpdate usageUpdate:
                    ...
                    await StudioSseWriter.WriteAsync(response, "usage", new { inputTokens, outputTokens }, cancellationToken);
                    break;
            ...
            switch (update)
            {
                ...
                case ChatTurnCompleted completed:
                    ...
                    await streamingTurnFinalizer.FinalizeCompletedAsync(
                        ...
                        async ct =>
                        {
                            await PersistConversationStateAsync(conversation, session.History, activeSkill, metadata, ct);
                            await TryGenerateConversationTitleAsync(conversation, userContent, assistantContent, ct);
                        },
                        cancellationToken);
                    return;
