...
test('create agent and stream chat reply', async ({ page }) => {
  ...
  await expect(page.getByText('Playwright Helper')).toBeVisible()

  await page.goto('/chat')
  const agentSelect = page.getByTestId('agent-select')
  const agentValue = await agentSelect.locator('option', { hasText: 'Playwright Helper' }).getAttribute('value')
